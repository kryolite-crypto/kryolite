...
public class Stake(IStoreRepository Repository, WalletCache Ledger, ValidatorCache Validators)
{
    ...
    public bool Lock(Address address, ulong value, out ExecutionResult executionResult, [NotNullWhen(true)] out Ledger? wallet)
    {
        if (!Ledger.TryGetWallet(address, Repository, out wallet))
        {
    ...
    public void Unlock(Address address, ulong value, [NotNull] out Ledger wallet)
    {
        if (!Ledger.TryGetWallet(address, Repository, out wallet!))
        {
            wallet = new Ledger(address);
