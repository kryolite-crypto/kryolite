...
public ref partial struct Serializer(ref byte spanRef, int length)
{
    ...
    public static byte[] SerializeN<T>(T? instance) where T : ISerializable, new()
    {
        if (instance is null)
        {
            var n = new T();
            return [n.GetSerializerId()];
        }
    ...
    public static T? DeserializeN<T>(ReadOnlySpan<byte> bytes) where T : ISerializable, new()
    {
        if (bytes.Length <= 1)
        {
            return default;
